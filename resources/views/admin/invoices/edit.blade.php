...
<div class="row invoice-add">
  ...
  <div class="col-lg-9 col-12 mb-lg-0 mb-4">
    <div class="card invoice-preview-card">
      <div class="card-body">
        <form action="{{ route('admin.invoices.update', $invoice->id) }}" method="POST" class="source-item" id="invoiceEditForm">
          @csrf
          @method('PUT')
          <div class="d-flex justify-content-between flex-column flex-sm-row px-sm-4 px-0">
            ...
            <div>
              <h4 class="fw-medium">EDIT INVOICE #{{ $invoice->invoice_number }}</h4>
              <div class="mb-2">
                ...
                <div class="d-flex border rounded position-relative pe-0">
                  <div class="row w-100 p-3">
                    ...
                    <div class="col-md-3 col-6 mb-md-0 mb-4">
                      ...
                      <input type="number" name="quantity" class="form-control invoice-item-qty" value="{{ $item['quantity'] }}" min="1" step="any" required />
                    </div>
                    <div class="col-md-3 col-6 mb-md-0 mb-4">
                      <p class="mb-2 repeater-title">Price</p>
                      <input type="number" name="price" class="form-control invoice-item-price" value="{{ $item['price'] }}" min="0" step="0.01" required />
                    </div>
                  </div>
            ...
            <div class="col-md-6 d-flex justify-content-end order-1 order-md-2">
              <div class="invoice-calculations">
                <div class="d-flex justify-content-between mb-2">
                  ...
                  <span class="fw-medium text-heading">$<span id="subtotalLabel">{{ number_format($invoice->subtotal, 2, '.', '') }}</span></span>
                </div>
                <div class="d-flex justify-content-between align-items-center mb-2 text-nowrap">
                  <span class="w-px-100">Tax:</span>
                  <input type="number" name="tax" id="taxInput" class="form-control form-control-sm w-px-100 ms-2" value="{{ $invoice->tax }}" step="0.01" />
                </div>
                <div class="d-flex justify-content-between align-items-center mb-2 text-nowrap">
                  <span class="w-px-100">Discount:</span>
                  <input type="number" name="discount" id="discountInput" class="form-control form-control-sm w-px-100 ms-2" value="{{ $invoice->discount }}" step="0.01" />
                </div>
                <hr />
                <div class="d-flex justify-content-between">
                  <span class="w-px-100 fw-bold">Total:</span>
                  <span class="fw-bold fs-5 text-heading">$<span id="totalLabel">{{ number_format($invoice->total, 2, '.', '') }}</span></span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="d-none">
            <input type="hidden" name="status" id="invoiceStatusHidden" value="{{ $invoice->status }}">
          </div>
        </form>
      </div>
    </div>
  ...
  <div class="col-lg-3 col-12 invoice-actions">
    <div class="card mb-4">
      <div class="card-body">
        <button type="button" class="btn btn-primary d-grid w-100 mb-3 waves-effect waves-light" onclick="document.getElementById('invoiceEditForm').submit()">
          <span class="d-flex align-items-center justify-content-center text-nowrap"><i class="ti ti-send ti-xs me-1"></i>Update Invoice</span>
        </button>
        <a href="{{ route('admin.invoices.show', $invoice->id) }}" class="btn btn-label-secondary d-grid w-100">Preview</a>
      </div>
    </div>
    <div>
      <div class="mt-4">
...
@section('page-js')
    <script src="{{ asset('admin-assets/js/app-invoice-edit.js') }}"></script>
    <script>
        $(function () {
